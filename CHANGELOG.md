...
### Fixed
* **Android RSA-OAEP decryption no longer depends on the provider-default MGF1 digest.** `decryptRsa` requested `RSA/ECB/OAEPWithSHA-256AndMGF1Padding` and called `init(DECRYPT_MODE, privateKey)` with no `AlgorithmParameterSpec`. That transformation name pins only the *main* digest — the MGF1 digest is left to the provider, and AndroidKeyStore silently supplies SHA-1 (unlike every other Android provider, which mirrors the main digest). The scheme protecting user data was therefore an undocumented platform default, and a future AOSP change to it would have made already-encrypted payloads undecryptable on updated devices with no way to recover them short of re-enrolling the key. The primary path now uses the bare `RSA/ECB/OAEPPadding` transformation and states both digests explicitly via `Constants.RSA_OAEP_SHA256_MGF1_SHA1` (`OAEPParameterSpec("SHA-256", "MGF1", MGF1ParameterSpec.SHA1, PSource.PSpecified.DEFAULT)`), which is what Android's own cryptography guidance recommends. SHA-1 is deliberate — it reproduces the parameters AndroidKeyStore has always applied, so this is behaviour-preserving and every existing ciphertext still decrypts. Fixes [#77](https://github.com/chamodanethra/biometric_signature/issues/77).
* **Android RSA decryption keeps two fallbacks so no key that decrypts today stops decrypting.** `openRsaDecryptCipher` now degrades in order: pinned OAEP → unparameterised `RSA/ECB/OAEPWithSHA-256AndMGF1Padding` (the pre-12.1.1 call, for any provider that rejects an explicit spec) → `RSA/ECB/PKCS1Padding` (keys created before 11.0.0, which only authorise PKCS#1 v1.5). The fallback trigger was also widened from `InvalidKeyException` to `GeneralSecurityException`, since supplying an `OAEPParameterSpec` introduces `InvalidAlgorithmParameterException` as a possible failure and the old `catch` would have let it escape as an unmapped error.

### Changed
* **Android RSA keys created with `enableDecryption: true` now authorise their OAEP MGF1 digest explicitly (API 35+).** `KeyGenParameterSpec.Builder.setMgf1Digests` was added in API 35; a key that does not call it inherits a platform default rather than a recorded value. `KeyManager.tryPinOaepMgf1Digest` declares `DIGEST_SHA1`, matching what decryption pins and what the platform has always used, so the guarantee holds on the key-authorisation side too and not just at cipher-init time. Best-effort and version-guarded — an unsupported builder call cannot fail key creation, and keys on API < 35 are unaffected (they carry no MGF1 authorisation at all).
* **Documented the exact RSA-OAEP parameters a backend must encrypt with.** README advertised `RSA/ECB/PKCS1Padding` in the feature list and "PKCS#1 padding" under the Android RSA key mode, both stale since 11.0.0 moved RSA decryption to OAEP. A new *Encrypting a payload* section replaces them with the real per-platform scheme, including the MGF1 asymmetry that is unavoidable because Android Keystore supports only SHA-1 for MGF1 while Apple's `.rsaEncryptionOAEPSHA256` uses SHA-256 for both digests — a backend has to branch on the platform it issued the public key to, and that was previously undocumented.
